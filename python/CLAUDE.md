...
- **Review before committing** — second-to-last task in every phase invokes the `code-review` skill (not a bash command — use the Skill tool) focused on architecture: class ownership, layer separation, method size, coupling. Apply high-confidence fixes before the commit.
